...
impl<T> Datastructure<T> for LinkedListDS<T> 
where T: PartialEq + ToString + std::fmt::Display + Clone + Send + Sync + 'static,
{

    // parallel_map verarbeitet parallel alle Elemente mit der Funktion 'f' und in 'target' gespeichert.
    fn parallel_map<U, F, D>(&self, f: F, target: D) -> D
    where
        F: Fn(&T) -> U + Send + Sync + + Copy + 'static, 
        U: Send + 'static,
        D: Datastructure<U>::parallel_map + Send + 'static,
    {
        parallel_map_impl(&self.data, f, target);
    }

    // Reduziert die Liste parallel mit der Funktion 'f' und einem 'initial' Wert.
    fn parallel_reduce<U, F>(&self, f: F, initial: U) -> U
    where 
        F: Fn(U, &T) -> U + Send + Sync + Copy + 'static,
        U: Send + 'static,
    {
        parallel_reduce_impl(&self.data, f, initial);
    }
